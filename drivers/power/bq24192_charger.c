 ...
 * add /sys entries
 */
static ssize_t chg_status_show(struct device *dev,
...
static ssize_t chg_status_store(struct device *dev,
...
DEVICE_ATTR(charge_status, 0644, chg_status_show, chg_status_store);
				  ...
				  const struct i2c_device_id *id)
{
	...
	ret = device_create_file(&client->dev, &dev_attr_charge_status);
	if (ret < 0) {
		pr_err("%s:File dev_attr_charge_status creation failed: %d\n",
				__func__, ret);
		ret = -ENODEV;
		goto err_at_charge;
	}
...
err_at_charge:
	device_remove_file(&client->dev, &dev_attr_charge_status);
